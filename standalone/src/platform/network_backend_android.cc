...
import std;
import platform.android.http_transport;
...
inline void start(std::string url, std::shared_ptr<Handle> handle,
  ...
  std::thread(
      [](std::string request, std::shared_ptr<Handle> progress,
         Callback done) {
        platform::android::http_transport::FetchResult result =
            platform::android::http_transport::fetch(
                request,
                [&progress](std::size_t received, std::size_t total) {
                  if (total > 0) {
                    progress->fProgress.store(
                        static_cast<float>(received) /
