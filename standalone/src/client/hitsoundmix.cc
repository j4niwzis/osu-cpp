...
[[nodiscard]] platform::audio::DecodedAudio
loadSample(std::string_view name, const osu::BeatmapSet &set,
  ...
  for (const std::string_view extension : {".wav", ".ogg"}) {
    ...
    if (!bytes.empty()) {
      return platform::audio::decodeAudio(bytes, extension);
    }
  }
  for (const std::string_view extension : {".wav", ".ogg"}) {
    ...
    return platform::audio::decodeAudio(detail::readFile(path), extension);
  }
  return {};
}

void addPcm(std::vector<float> &mix, int outputRate, std::size_t atFrame,
            const platform::audio::DecodedAudio &sample, float gain) {
...
mixReplayAudio(std::span<const std::uint8_t> musicBytes,
  ...
  auto music = platform::audio::decodeAudio(musicBytes, musicExtension);
  ...
  std::unordered_map<std::string, platform::audio::DecodedAudio> samples;
